...
class DeleteCredentialObservable(context: Context, val credential: Credential) : BaseCredentialObservable<Boolean>(context) {
    ...
    override fun getGoogleApiClientCallback(subscriber: SingleEmitter<in Boolean>): BaseGoogleApiClientCallback<Boolean> {
        return object : BaseGoogleApiClientCallback<Boolean>(subscriber) {
            ...
            override fun onConnected(p0: Bundle?) {
                try {
                    Auth.CredentialsApi.delete(googleApiClient, credential).setResultCallback(this)
                } catch (e: Exception) {
